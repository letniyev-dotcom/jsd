...
class AppState extends ChangeNotifier {
  ...
  /// (см. lib/screens/changelog.dart). При пуше добавляется телом
  /// коммита (после заголовка) — именно поэтому он потом виден в
  /// деталях сборки (Run) на вкладке Actions: GitHub возвращает полный
  /// текст коммита в head_commit.message.
  ///
  /// Внутри текста упоминания багов (см. `#` в редакторе) хранятся как
  /// служебный символ-плейсхолдер `\uFFFC` — по одному на упоминание,
  /// по порядку слева направо. Какому багу соответствует каждый
  /// плейсхолдер — смотри [changelogMentions] (тот же порядок).
  String changelog = '';

  /// id багов (см. [BugItem.id]) для каждого `\uFFFC`-плейсхолдера в
  /// [changelog], по порядку. См. `expandChangelogText` в
  /// lib/screens/changelog.dart — превращает плейсхолдеры в читаемый
  /// текст вида «#1234 (Название)» перед отправкой в коммит.
  List<String> changelogMentions = [];
